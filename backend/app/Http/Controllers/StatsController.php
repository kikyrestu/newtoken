...
class StatsController extends Controller
{
    ...
    public function getGlobalStats(): JsonResponse
    {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'cumulative_burn_formatted' => $totalLockedFormatted, // Alias for frontend
    ...
    /**
     * Get countdown version for real-time sync polling
     * GET /api/countdown-version
     * 
     * Returns a hash that changes when any lock transaction is updated.
     * Clients can poll this endpoint to detect changes without fetching full data.
     */
    public function getCountdownVersion(): JsonResponse
    {
        // Get the most recent lock transaction update timestamp
        $latestUpdate = LockTransaction::max('updated_at');
        
        // Also check for any new locks
        $latestCreate = LockTransaction::max('created_at');
        
        // Create version hash from both timestamps
        $version = md5(($latestUpdate ?? '') . '|' . ($latestCreate ?? '') . '|' . now()->format('Y-m-d-H'));
