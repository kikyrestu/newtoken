...
class PriceOracleService
{
    ...
    public function __construct()
    {
        ...
        $dbDecimals = AppSetting::get('token_decimals');
        $this->tokenDecimals = $dbDecimals !== null 
            ? (int) $dbDecimals 
            : (int) config('solana.token_decimals', 9);
            
        Log::debug("[PriceOracle] Initialized with token: {$this->tokenMint}, decimals: {$this->tokenDecimals}");
    }
    ...
    public function getTokenPriceUSD(): float
    {
        ...
        // DEV MODE: Return mock price for devnet tokens not on price oracles
        ...
        if ($devMode || !in_array($this->tokenMint, $knownMainnetTokens)) {
            // Return mock price for testing: $0.01 per token
            Log::info("[PriceOracle] DEV MODE: Using mock price $0.01 for token {$this->tokenMint}");
            return 0.01;
        }
    ...
    private function fetchPriceFromRaydium(): float
    {
        try {
            ...
            // SECURITY: Only bypass SSL in local development
            $http = Http::timeout(15);
            if (config('app.env') === 'local') {
                $http = $http->withoutVerifying();
            }
            
            $response = $http->get('https://api-v3.raydium.io/mint/price', [
    ...
    private function fetchPriceFromJupiter(): float
    {
        try {
            ...
            $response = $http->get('https://api.jup.ag/price/v2', [
                    'ids' => $this->tokenMint
                ]);
            ...
            $http4 = Http::timeout(15);
            if (config('app.env') === 'local') {
                $http4 = $http4->withoutVerifying();
            }
            
            $response = $http4->get('https://price.jup.ag/v4/price', [
                    'ids' => $this->tokenMint
                ]);
