...
export const CountdownTimer: React.FC<CountdownTimerProps> = ({
     ...
     * Falls back to local time if RPC is unavailable (e.g., mainnet blocks browser requests)
     */
    const syncBlockchainTime = useCallback(async () => {
        // In development or when RPC is blocked, use local time immediately
        const useFallback = () => {
        ...
        try {
            ...
            // Silently fallback to local time - no console spam
            useFallback();
        }
    }, [connection]);
    ...
    useEffect(() => {
        ...
        const updateCountdown = () => {
            ...
            // Calculate raw time values
            const rawDays = Math.floor(diff / (60 * 60 * 24));
            const rawHours = Math.floor((diff % (60 * 60 * 24)) / (60 * 60));
            const rawMinutes = Math.floor((diff % (60 * 60)) / 60);
            const rawSeconds = diff % 60;

            // Cap at maximum 99 days, 23 hours, 59 minutes, 59 seconds
            const MAX_DAYS = 99;
            if (rawDays > MAX_DAYS) {
                setTimeLeft({
                    days: MAX_DAYS,
