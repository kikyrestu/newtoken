...
import { Shield, BookOpen, Activity, FileText } from 'lucide-react';

const API_BASE_URL = import.meta.env.VITE_API_URL || 'http://localhost:8000/api';

type StatsData = {
    total_locked_formatted: string;
    cumulative_burn_formatted?: string;
    active_locks: number;
        ...
        <div className="relative flex flex-col gap-4 w-[280px] animate-slide-in-left">
            ...
            <div className="relative bg-black/10 backdrop-blur-sm border border-white/10 rounded-lg overflow-hidden">
                ...
                <div className="grid grid-cols-[1fr_auto] gap-x-4 gap-y-2 p-3 text-xs">
                    {/* Row 1: Cumulative Burn */}
                    <div className="text-gray-400 uppercase tracking-wide flex items-center">Cumulative Burn</div>
                    <div className="font-mono font-bold text-[#00ff41] text-right">
                        {loading ? '---' : stats?.cumulative_burn_formatted || stats?.total_locked_formatted || '0'}
                    </div>

                    {/* Row 2: Active Locks (Total Token Amount) */}
                    ...
                    {/* Participants Header */}
                    <div className="col-span-2 mt-2 mb-1 uppercase text-[10px] text-gray-500 tracking-widest border-t border-white/10 pt-2">Participants</div>

                    {/* Observer slots */}
                    <div className="text-gray-400 uppercase tracking-wide flex items-center text-[11px]">Observer</div>
                    <div className="font-mono font-bold text-[#00ff41] text-right text-[11px]">
                        {loading ? '--' : `${stats?.tier_breakdown?.spectators || 0}/${stats?.tier_limits?.observer || 1000}`}
                    </div>

                    {/* Operator slots */}
                    <div className="text-gray-400 uppercase tracking-wide flex items-center text-[11px]">Operator</div>
                    <div className="font-mono font-bold text-[#00ff41] text-right text-[11px]">
                        {loading ? '--' : `${stats?.tier_breakdown?.operators || 0}/${stats?.tier_limits?.operator || 700}`}
                    </div>

                    {/* Elite slots */}
                    <div className="text-gray-400 uppercase tracking-wide flex items-center text-[11px]">Elite</div>
                    <div className="font-mono font-bold text-[#00ff41] text-right text-[11px]">
                        {loading ? '--' : `${stats?.tier_breakdown?.elite || 0}/${stats?.tier_limits?.elite || 300}`}
                    </div>
                </div>
            ...
            <div className="flex flex-col gap-2 pl-1">
                ...
                >
                    ...
                    {/* Icon & Text */}
                    <FileText className="w-4 h-4 text-[#00ff41] relative z-10" />
                    <span className="text-xs font-bold tracking-widest uppercase relative z-10">Blueprint PDF</span>
