                    ...
                    <main className="flex-1 flex flex-col justify-start min-h-0 pointer-events-auto">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center px-4 pointer-events-none z-[5]">
                            {!showSafetyModal ? (
                                ...
                                <div className="w-full max-w-md transition-all duration-500 pointer-events-auto">
                                    {/* Styled Modal - Matching MissionTierCard Style */}
                                    <div className="relative bg-[#0a0c10]/90 border border-[#4fffa0] p-4 rounded-lg 
                                        transition-all duration-300 overflow-hidden backdrop-blur-md
                                        shadow-[0_0_30px_rgba(79,255,160,0.4)]">

                                        {/* Top Green Line Accent - Same as cards */}
                                        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1/3 h-[3px] bg-[#4fffa0] shadow-[0_0_15px_#4fffa0] rounded-full" />

                                        {/* Tier Badge Style Header */}
                                        <div className="absolute top-3 right-3">
                                            <span className="text-xs px-2 py-1 rounded-full font-bold uppercase tracking-wider bg-green-500/20 text-green-400">
                                                ACTIVE
                                            </span>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-3 mb-4 pr-16">
                                            <Shield className="w-5 h-5 text-[#4fffa0]" />
                                            <h2 className="text-base font-bold text-white uppercase tracking-wider">
                                                Safety Protocols
                                            </h2>
                                            <button
                                                onClick={() => setShowSafetyModal(false)}
                                                className="absolute top-3 right-14 p-1.5 text-gray-400 hover:text-white hover:bg-white/10 rounded transition-colors"
                                            >
                                                <X size={16} />
                                            </button>
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            {[
                                                ...
                                                <div key={index} className="flex items-start gap-2 p-2 bg-black/30 rounded border border-white/5 hover:border-[#4fffa0]/30 transition-colors">
                                                    <span className="inline-block w-1.5 h-1.5 bg-[#4fffa0] rounded-full mt-1.5 flex-shrink-0 animate-pulse" />
                                                    <div>
                                                        <span className="text-sm font-bold text-[#4fffa0]">{item.title}</span>
                                        ...
                                        {/* Decorative Corner - Same as cards */}
