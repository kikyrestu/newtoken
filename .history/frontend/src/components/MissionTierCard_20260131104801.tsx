...
export const MissionTierCard: React.FC<MissionTierCardProps> = ({
    ...
    const { lockTokens, getTokenBalance, formatTokenAmount, getSolscanUrl, loading, error: lockError } = useLockProgram();
    const { pricing, formatTokens, isPriceAvailable } = useTokenPrice();

    // Initialize state based on props
    const [status, setStatus] = useState<CardStatus>(isLocked ? 'success' : 'idle');
    const [txSignature, setTxSignature] = useState<string | null>(lockedSignature);
    const [errorMessage, setErrorMessage] = useState<string | null>(null);

    // Sync state if props change (e.g. after dashboard load)
    React.useEffect(() => {
        if (isLocked) {
            setStatus('success');
            if (lockedSignature) setTxSignature(lockedSignature);
        }
    }, [isLocked, lockedSignature]);
    ...
    const getCardBorderClass = () => {
        switch (status) {
            case 'success': return 'border-[#00ff41] shadow-[0_0_30px_rgba(0,255,65,0.4)]';
            case 'error': return 'border-red-500 shadow-[0_0_20px_rgba(255,0,0,0.2)]';
            default: return 'border-[#333] hover:border-[#00ff41]';
        }
    };
        ...
        <div className="group cursor-pointer flex flex-col items-center">
            ...
            >
                ...
                <h3 className="text-white font-bold text-base md:text-lg mb-2 pr-24">
                    ...
                    {tierPricing && tierPricing.discount_percent > 0 ? (
                        <span className="ml-2">
                            <span className="line-through text-gray-500 text-sm">${tierPricing.original_price}</span>
                            <span className="text-[#00ff41] ml-1">${tierPricing.current_price}</span>
                            <span className="ml-1 text-xs bg-red-500/20 text-red-400 px-1.5 py-0.5 rounded">
                                -{tierPricing.discount_percent}%
                            </span>
                        </span>
                    ) : (
                        <span className="text-[#00ff41] ml-2">${tierPricing?.usd ?? tierInfo.usd}</span>
                    )}
                </h3>

                {/* Urgency Indicator */}
                {tierPricing && tierPricing.discount_percent > 0 && tierPricing.remaining_slots_in_tier !== null && tierPricing.remaining_slots_in_tier <= 50 && (
                    <div className="mb-2 px-2 py-1 bg-orange-500/10 border border-orange-500/30 rounded text-[10px] text-orange-400 animate-pulse">
                        🔥 Only {tierPricing.remaining_slots_in_tier} spots left at -{tierPricing.discount_percent}%!
                    </div>
                )}
                ...
                {tierPricing && isPriceAvailable(tier) && (
                    <div className="text-xs text-[#00ff41]/70 mb-3 flex items-center gap-1">
                ...
                {txSignature && status === 'success' && (
                    <a
                        ...
                        className="text-xs text-[#00ff41]/50 hover:text-[#00ff41] mt-3 block text-center transition-colors"
                    >
