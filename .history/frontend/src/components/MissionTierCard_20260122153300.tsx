...
import { useLockProgram, TierType, TIERS } from '../hooks/useLockProgram';
...
export const MissionTierCard: React.FC<MissionTierCardProps> = ({
    ...
    return (
        <div className="group cursor-pointer flex flex-col items-center">
            {/* Number above card */}
            <span className="text-2xl text-white/50 mb-3 font-serif">{cardNumber}</span>

            {/* Card Body */}
            <div
                className={`
                    w-full bg-[#0a0c10]/90 border p-6 rounded-xl 
            ...
            >
                ...
                {/* Content */}
                <h3 className="text-white font-bold text-xl mb-2 pr-24">
                    {title}
                ...
                {tierPricing && isPriceAvailable(tier) && (
                    <div className="text-xs text-[#4fffa0]/70 mb-4 flex items-center gap-2">
                        <span className="inline-block w-2 h-2 bg-[#4fffa0] rounded-full animate-pulse" />
                ...
                {/* Action Button */}
                <button
                    onClick={status === 'error' ? handleRetry : handleBuyClick}
                    disabled={loading || status === 'success' || status === 'confirming'}
                    className={`
                ...
                <div className="mt-4 pt-4 border-t border-white/10 flex justify-between text-xs text-gray-500">
                    ...
                    <span className="text-white/70">{tierInfo.durationDays} Days</span>
                </div>
