...
export const CountdownTimer: React.FC<CountdownTimerProps> = ({
    ...
    if (!unlockTimestamp) {
        return (
            <div className={`countdown-display ${className}`}>
                <div className={`
                    ${sizeClasses.digits} font-orbitron font-bold text-[#00ff41]/30 tabular-nums
                    tracking-wider
                `}>
                    00:00:00:00
                </div>
    ...
    return (
        <div className={`countdown-display ${className}`}>
            {variant === 'device' ? (
                <div
                    className="w-full h-full flex items-center justify-center text-4xl xl:text-4xl font-orbitron font-bold text-[#00ff41] tabular-nums tracking-tight drop-shadow-[0_0_10px_rgba(0,255,65,0.6)]"
                >
