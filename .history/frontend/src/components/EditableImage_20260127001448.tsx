...
interface EditableImageProps {
    ...
    alt?: string;
    className?: string;
    storageKey?: string; // For persisting transforms
    initialWidth?: number;
...
export const EditableImage = ({
    ...
    const [transform, setTransform] = useState<ImageTransform>(() => {
        if (storageKey) {
            const saved = localStorage.getItem(`editableImage_${storageKey}`);
            if (saved) {
                try {
        ...
        return { width: initialWidth, height: initialHeight, x: 0, y: 0 };
    });
    ...
    const saveTransform = useCallback((newTransform: ImageTransform) => {
        if (storageKey) {
            localStorage.setItem(`editableImage_${storageKey}`, JSON.stringify(newTransform));
        }
    }, [storageKey]);
    ...
    // Reset to original
    const handleReset = () => {
        const resetTransform = { width: initialWidth, height: initialHeight, x: 0, y: 0 };
        setTransform(resetTransform);
        if (storageKey) {
            localStorage.removeItem(`editableImage_${storageKey}`);
        }
        setIsEditing(false);
    };
