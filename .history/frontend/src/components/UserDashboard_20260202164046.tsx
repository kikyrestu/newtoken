...
export const UserDashboard: React.FC = () => {
    ...
    const getRankInfo = (points: number) => {
        if (points >= 600) return { name: 'Elite 🟡', color: 'text-yellow-400', nextThreshold: null, pointsToNext: 0 };
        if (points >= 300) return { name: 'Veteran 🟣', color: 'text-purple-400', nextThreshold: 600, pointsToNext: 600 - points };
        if (points >= 100) return { name: 'Operator 🔵', color: 'text-blue-400', nextThreshold: 300, pointsToNext: 300 - points };
        return { name: 'Novice 🟢', color: 'text-green-400', nextThreshold: 100, pointsToNext: 100 - points };
    };
                            ...
                            <div className="border border-gray-700 rounded-lg p-4 bg-black/30">
                                <div className="flex items-center justify-between mb-3">
                                    <div>
                                        ...
                                        <p className={`text-lg font-bold ${rankInfo.color}`}>{rankInfo.name}</p>
                                    </div>
                                    <div className="text-right">
                                        <span className="text-gray-500 text-xs">Points to next rank</span>
                                        <p className="text-white font-mono">{rankInfo.nextThreshold ? rankInfo.pointsToNext : 'MAX'}</p>
                                    </div>
                                </div>
                    ...
                    {topTab === 'symbol' && (
                        <div className="flex flex-col items-center justify-center h-full">
                            ...
                            <h3 className="text-lg font-bold text-white uppercase tracking-widest mb-2">
                                LIVE Access <span className="text-[#00ff41]">Coming Soon</span>
                            </h3>
