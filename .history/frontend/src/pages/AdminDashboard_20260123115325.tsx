...
const AdminDashboard = () => {
    ...
    // Fetch stats on auth success or mount if already auth
    const fetchStats = async () => {
        setLoading(true);
        try {
            const response = await fetch('http://localhost:8000/api/admin/overview', {
                headers: {
                    'X-Admin-Token': 'admin-secret-123', // Matches backend .env
            ...
            <main className="flex-1 overflow-auto">
                ...
                <div className="p-6 md:p-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {[
                            { label: 'Total Revenue', value: `$${stats.revenue_usd.toLocaleString()}`, icon: '$' },
                            { label: 'Total Users', value: stats.total_users.toString(), icon: 'Active' },
                            { label: 'Active Locks', value: stats.active_locks.toString(), icon: '🔒' }
                        ].map((stat, i) => (
                            <div key={i} className="bg-[#0f1216] border border-[#4fffa0]/10 rounded-xl p-6 relative overflow-hidden group">
                                ...
                                <div className="mt-4 flex items-center text-xs text-[#4fffa0]">
                                    <span className="bg-[#4fffa0]/10 px-2 py-0.5 rounded mr-2">
                                        {i === 2 ? `+${stats.recent_activity}` : '+0%'}
                                    </span>
                                    vs last 7 days
                                </div>
                            </div>
                    ...
                    <div className="bg-[#0f1216] border border-[#4fffa0]/10 rounded-xl overflow-hidden min-h-[400px] flex items-center justify-center text-gray-500">
                        <div className="text-center">
                            ...
                            <p>Real-time data stream connected.</p>
                            <p className="text-sm mt-2 text-[#4fffa0]/50">Waiting for transaction events...</p>
                        </div>
                    </div>
                </div>
            </main>
