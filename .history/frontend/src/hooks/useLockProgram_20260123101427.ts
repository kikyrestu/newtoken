...
export const TIERS = {
    spectator: { usd: 20, durationDays: 30, name: 'Spectator' },
...
export function useLockProgram() {
    ...
    const getEscrowPDA = useCallback((userPubkey: PublicKey): [PublicKey, number] => {
        // In Demo Mode, return a dummy PDA if needed, but real derivation is fine too
        // as long as we don't query it on-chain
    ...
    // ... (keep helper functions like getEscrowTokenAccount)
    ...
    const lockTokens = useCallback(async (
        ...
        try {
            ...
            if (DEMO_MODE) {
                ...
                // Call backend mock verify if needed, or just return success
                // For now, we assume backend also accepts demo signatures or we skip backend verification in demo
