...
export function useLockProgram() {
    ...
    const lockTokens = useCallback(async (
        ...
        try {
            ...
            // 4. Verify Program ID is configured (ESCROW ONLY MODE)
            const isPlaceholderProgram = PROGRAM_ID.toBase58() === '11111111111111111111111111111111';
            if (isPlaceholderProgram) {
                throw new Error('Smart contract not configured. Please configure Program ID in admin panel.');
            }

            // 5. Build transaction
            const tx = new Transaction();

            // 6. Create escrow token account if needed
            ...
            // 7. Create lock instruction (ESCROW MODE)
